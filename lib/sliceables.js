...
export default class Sliceables {
  ...
  handleSliceables(circle, time) {
    ...
    .call(createjs.Ticker.addEventListener("tick", this.tick))
  ...
  tick(event) {
    ...
    Object.keys(this.circles).forEach((id) =>{
      ...
      if (self.circles[id].x+30 > self.width || self.circles[id].y > self.height) { self.circles[id].x = 0;
        self.circles[id].mouseEnabled = false;
        self.stage.removeChild(self.circles[id].shape);
        // self.stage.circles[id].removeAllEventListeners();
